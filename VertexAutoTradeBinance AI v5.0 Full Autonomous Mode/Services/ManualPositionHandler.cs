...
{
    /// <summary>
    /// ManualPositionHandler (ПРОПАТЧЕНО)
    ///
    /// Исправления:
    /// - Защита идемпотентности: ручной сигнал создаётся ТОЛЬКО ОДИН РАЗ
    ///   для комбинации (symbol + side + abs(qty) + entry)
    /// - Предыдущее состояние обновляется для сценариев,
    ///   где детектор закрытия позиции опирается на prev-state
    /// - Анти-зацикливание памяти: если последний сигнал был ручным,
    ///   он не пересоздаётся на каждом тике
    ///
    /// Примечания:
    /// - Логика PositionSupervisorService НЕ изменяется
    /// - AI-сигналы не блокируются глобально; предотвращается
    ///   только спам ручных сигналов
    /// </summary>
