      ...
      CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (IsParabolicMove(klines))
                {
                    ...
                    _logger.LogInformation("[PARABOLIC] {symbol} {tf} conf adjusted {conf:F2}", symbol, tf, finalConfidence);
                }
                ...
                if (IsAgainstMicroMomentum(klines, baseSignal.Side))
                {
                    ...
                    _logger.LogInformation("[MICRO_MOMENTUM] {symbol} {tf} conf adjusted {conf:F2}", symbol, tf, finalConfidence);
                }
                ...
                _logger.LogInformation("[TRAP] {symbol} {tf} mult={mult:F2} conf={conf:F2}", symbol, tf, trapMult, finalConfidence);

                // Bad entry location (soft)
